...
bool vector_new(vector_t *self, size_t element_size, deleter_t deleter, const allocator_t *allocator) {
    ...
    void *array = allocator_calloc(allocator, VECTOR_DEFAULT_CAP, element_size);
    if (array == NULL) {
        return false;
    }
    *self = (vector_t){
...
bool vector_copy(vector_t *self, const vector_t *copied) {
    ...
    void *array = allocator_calloc(copied->allocator, copied->capacity, copied->element_size);
    if (array == NULL) {
        return false;
    }
...
bool vector_resize(vector_t *self, size_t capacity) {
    ...
    if (self->deleter != NULL && self->count > capacity) {
        deleted = allocator_malloc(self->allocator, (self->count - capacity) * self->element_size);
        if (deleted == NULL) {
            return false;
        }
        memcpy(
    ...
    void *array = allocator_realloc(self->allocator, self->array, capacity * self->element_size);
    if (array == NULL) {
...
bool vector_foreach(vector_t *self, bool (*action)(void *element, void *data), void *data) {
    ...
    assert(action != NULL);
    for (size_t i = 0; i < self->count; ++i) {
        if (!action(vector_get(self, i), data)) {
            return false;
        }
    }
    return true;
...
bool vector_foreach_const(const vector_t *self, bool (*action)(const void *element, void *data), void *data) {
    ...
    assert(action != NULL);
    for (size_t i = 0; i < self->count; ++i) {
        if (!action(vector_get_const(self, i), data)) {
            return false;
        }
    }
    return true;

...
bool string_new(string_t *self, const char *str, const allocator_t *allocator) {
    assert(self != NULL);
    assert(str != NULL);
    allocator = allocator == NULL ? &stdalloc : allocator;
    size_t length = strlen(str);
    size_t capacity = (length + 1) > STRING_DEFAULT_CAP ? (length + 1) : STRING_DEFAULT_CAP;
    char *buffer = (char *) allocator_calloc(allocator, capacity, 1);
    if (buffer == NULL) {
        return false;
    }
...
bool string_copy(string_t *self, const string_t *copied) {
    ...
    char *buffer = (char *) allocator_calloc(copied->allocator, capacity, 1);
    if (buffer == NULL) {
        return false;
    }
...
bool string_resize(string_t *self, size_t length) {
    ...
    char *buffer = allocator_realloc(self->allocator, self->buffer, capacity);
    if (buffer == NULL) {
        return false;
    }
...
const char *string_format(string_t *self, ...) {
    ...
    char *format = (char *) allocator_malloc(self->allocator, self->length + 1);
    if (format == NULL) {
        return NULL;
    }
    ...
    while (length >= self->capacity) {
        if (!string_resize(self, (self->capacity * STRING_EXPANSION) - 1)) {
            allocator_free(self->allocator, format);
            return NULL;
        }
    }
    va_start(args, self);
...
size_t string_total(const string_t *self, const char *str) {
    assert(self != NULL);
    assert(self->capacity > 0);
    assert(self->length < self->capacity);
    assert(self->buffer != NULL);
...
const char *string_replace_first(string_t *self, const char *find, const char *replace) {
    ...
    char *copy = (char *) allocator_malloc(self->allocator, self->length + 1);
    if (copy == NULL) {
        return NULL;
    }
    ...
    allocator_free(self->allocator, copy);
    return self->buffer;
}
...
bool string_foreach(const string_t *self, bool (*action)(char element, void *data), void *data) {
    assert(self != NULL);
    assert(self->capacity > 0);
    assert(self->length < self->capacity);
    assert(self->buffer != NULL);

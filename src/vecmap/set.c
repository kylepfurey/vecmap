// .c
// Binary Tree Set
// by Kyle Furey

#include <vecmap/set.h>
#include <string.h>
#include <assert.h>

static bool tree_copy(const tree_t *self, set_t *set) {
    if (self == NULL) {
        return true;
    }
    assert(self->data != NULL);
    if (!tree_copy(self->left, set)) {
        return false;
    }
    if (!tree_copy(self->right, set)) {
        return false;
    }
    return set_insert(set, self->data) != NULL;
}

static void tree_clear(tree_t *self, deleter_t deleter, const allocator_t *allocator) {
    if (self == NULL) {
        return;
    }
    assert(self->data != NULL);
    tree_clear(self->left, deleter, allocator);
    if (deleter != NULL) {
        deleter(self->data);
    }
    allocator_free(allocator, self->data);
    self->data = NULL;
    tree_t *right = self->right;
    allocator_free(allocator, self);
    tree_clear(right, deleter, allocator);
}

static bool tree_foreach(const tree_t *self, bool (*action)(const void *element, void *data), void *data) {
    assert(action != NULL);
    if (self == NULL) {
        return true;
    }
    assert(self->data != NULL);
    if (!tree_foreach(self->left, action, data)) {
        return false;
    }
    if (!action(self->data, data)) {
        return false;
    }
    return tree_foreach(self->right, action, data);
}

bool set_new(set_t *self, size_t element_size, comparer_t comparer, deleter_t deleter, const allocator_t *allocator) {
    assert(self != NULL);
    assert(element_size > 0);
    allocator = allocator == NULL ? &stdalloc : allocator;
    *self = (set_t){
        element_size,
        0,
        NULL,
        comparer,
        deleter,
        allocator,
    };
    return true;
}

bool set_copy(set_t *self, const set_t *copied) {
    assert(self != NULL);
    assert(copied != NULL);
    assert(self != copied);
    assert(copied->element_size > 0);
    assert((copied->count == 0) == (copied->root == NULL));
    set_new(self, copied->element_size, copied->comparer, copied->deleter, copied->allocator);
    if (!tree_copy(copied->root, self)) {
        set_clear(self);
        return false;
    }
    return true;
}

size_t set_count(const set_t *self) {
    assert(self != NULL);
    assert(self->element_size > 0);
    assert((self->count == 0) == (self->root == NULL));
    return self->count;
}

bool set_empty(const set_t *self) {
    assert(self != NULL);
    assert(self->element_size > 0);
    assert((self->count == 0) == (self->root == NULL));
    return self->count == 0;
}

const void *set_least(const set_t *self) {
    assert(self != NULL);
    assert(self->element_size > 0);
    assert((self->count == 0) == (self->root == NULL));
    const tree_t *current = self->root;
    if (current == NULL) {
        return NULL;
    }
    while (current->left != NULL) {
        current = current->left;
    }
    return current->data;
}

const void *set_greatest(const set_t *self) {
    assert(self != NULL);
    assert(self->element_size > 0);
    assert((self->count == 0) == (self->root == NULL));
    const tree_t *current = self->root;
    if (current == NULL) {
        return NULL;
    }
    while (current->right != NULL) {
        current = current->right;
    }
    return current->data;
}

const void *set_find(const set_t *self, const void *element) {
    assert(self != NULL);
    assert(self->element_size > 0);
    assert((self->count == 0) == (self->root == NULL));
    assert(element != NULL);
    const tree_t *current = self->root;
    while (current != NULL) {
        int result;
        if (self->comparer != NULL) {
            result = (int) self->comparer(element, current->data);
        } else {
            result = memcmp(element, current->data, self->element_size);
        }
        if (result == 0) {
            return current->data;
        } else if (result < 0) {
            current = current->left;
        } else {
            current = current->right;
        }
    }
    return NULL;
}

bool set_contains(const set_t *self, const void *element) {
    assert(self != NULL);
    assert(self->element_size > 0);
    assert((self->count == 0) == (self->root == NULL));
    return set_find(self, element) != NULL;
}

const void *set_insert(set_t *self, const void *element) {
    assert(self != NULL);
    assert(self->element_size > 0);
    assert((self->count == 0) == (self->root == NULL));
    assert(element != NULL);
    tree_t **current = &self->root;
    while (*current != NULL) {
        int result;
        if (self->comparer != NULL) {
            result = (int) self->comparer(element, (*current)->data);
        } else {
            result = memcmp(element, (*current)->data, self->element_size);
        }
        if (result == 0) {
            if (self->deleter != NULL) {
                self->deleter((*current)->data);
            }
            memcpy((*current)->data, element, self->element_size);
            return (*current)->data;
        }
        current = (result < 0) ? (&(*current)->left) : (&(*current)->right);
    }
    tree_t *tree = (tree_t *) allocator_malloc(self->allocator, sizeof(tree_t));
    if (tree == NULL) {
        return NULL;
    }
    void *data = allocator_malloc(self->allocator, self->element_size);
    if (data == NULL) {
        allocator_free(self->allocator, tree);
        return NULL;
    }
    memcpy(data, element, self->element_size);
    *tree = (tree_t){
        data,
        NULL,
        NULL,
        self,
    };
    *current = tree;
    self->count++;
    return tree->data;
}

bool set_erase(set_t *self, const void *element) {
    assert(self != NULL);
    assert(self->element_size > 0);
    assert((self->count == 0) == (self->root == NULL));
    assert(element != NULL);
    tree_t **current = &self->root;
    while (*current != NULL) {
        int result;
        if (self->comparer != NULL) {
            result = (int) self->comparer(element, (*current)->data);
        } else {
            result = memcmp(element, (*current)->data, self->element_size);
        }
        if (result == 0) {
            break;
        }
        current = (result < 0) ? (&(*current)->left) : (&(*current)->right);
    }
    if (*current == NULL) {
        return false;
    }
    tree_t *target = *current;
    if (target->left == NULL) {
        *current = target->right;
    } else if (target->right == NULL) {
        *current = target->left;
    } else {
        tree_t **successor = &target->right;
        while ((*successor)->left != NULL) {
            successor = &(*successor)->left;
        }
        void *swap = target->data;
        target->data = (*successor)->data;
        (*successor)->data = swap;
        target = *successor;
        *successor = target->right;
    }
    if (self->deleter != NULL) {
        self->deleter(target->data);
    }
    allocator_free(self->allocator, target->data);
    target->data = NULL;
    allocator_free(self->allocator, target);
    self->count--;
    return true;
}

void set_clear(set_t *self) {
    assert(self != NULL);
    assert(self->element_size > 0);
    assert((self->count == 0) == (self->root == NULL));
    tree_clear(self->root, self->deleter, self->allocator);
    self->count = 0;
    self->root = NULL;
}

bool set_foreach(const set_t *self, bool (*action)(const void *element, void *data), void *data) {
    assert(self != NULL);
    assert(self->element_size > 0);
    assert((self->count == 0) == (self->root == NULL));
    assert(action != NULL);
    return tree_foreach(self->root, action, data);
}

void set_free(set_t *self) {
    assert(self != NULL);
    assert(self->element_size > 0);
    assert((self->count == 0) == (self->root == NULL));
    set_clear(self);
    *self = (set_t){0};
}

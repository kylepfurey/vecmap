...
#include <vecmap/map.h>
#include <stdint.h>
#include <string.h>
#include <assert.h>

static size_t map_hash_default(const void *data, size_t size) {
    assert(data != NULL);
...
static bool map_equate_default(const void *left, const void *right, size_t size) {
    assert(left != NULL);
    assert(right != NULL);
...
bool map_new(map_t *self, size_t key_size, size_t value_size, hasher_t hasher, equater_t equater, deleter_t value_deleter, const allocator_t *allocator) {
    ...
    pair_t *pairs = (pair_t *) allocator_calloc(allocator, MAP_DEFAULT_CAP, sizeof(pair_t));
    if (pairs == NULL) {
        return false;
    }
...
bool map_copy(map_t *self, const map_t *copied) {
    ...
    pair_t *pairs = (pair_t *) allocator_malloc(copied->allocator, copied->capacity * sizeof(pair_t));
    if (pairs == NULL) {
        return false;
    }
    for (size_t i = 0; i < copied->capacity; ++i) {
        ...
        pairs[i].key = allocator_malloc(copied->allocator, copied->key_size);
        if (pairs[i].key == NULL) {
            for (size_t j = 0; j < i; ++j) {
                if (pairs[j].state != PAIR_STATE_FULL) {
                    continue;
                }
                allocator_free(copied->allocator, pairs[j].key);
                allocator_free(copied->allocator, pairs[j].value);
            }
            allocator_free(copied->allocator, pairs);
            return false;
        }
        memcpy(pairs[i].key, copied->pairs[i].key, copied->key_size);
        pairs[i].value = allocator_malloc(copied->allocator, copied->value_size);
        if (pairs[i].value == NULL) {
            allocator_free(copied->allocator, pairs[i].key);
            for (size_t j = 0; j < i; ++j) {
                if (pairs[j].state != PAIR_STATE_FULL) {
                    continue;
                }
...
bool map_resize(map_t *self, size_t capacity) {
    ...
    pair_t *pairs = (pair_t *) allocator_calloc(self->allocator, capacity, sizeof(pair_t));
    if (pairs == NULL) {
        return false;
    }
    ...
    allocator_free(self->allocator, self->pairs);
    self->pairs = pairs;
...
void *map_insert(map_t *self, const void *key) {
    ...
    if (self->count * MAP_LOAD_FACTOR_DEN >= self->capacity * MAP_LOAD_FACTOR_NUM) {
        ...
        if (!map_resize(self, self->capacity * MAP_EXPANSION)) {
            return NULL;
        }
    }
    size_t hash;
    ...
    for (size_t i = 0; i < self->capacity; ++i) {
        ...
        switch (pair->state) {
            case PAIR_STATE_EMPTY:
            case PAIR_STATE_SKIP:
                new_key = allocator_malloc(self->allocator, self->key_size);
                if (new_key == NULL) {
                    return NULL;
                }
                new_value = allocator_malloc(self->allocator, self->value_size);
                if (new_value == NULL) {
                    allocator_free(self->allocator, new_key);
                    return NULL;
                }
            ...
            case PAIR_STATE_FULL:
                if (self->equater != NULL) {
                    if (self->equater(key, pair->key)) {
                        ...
                        allocator_free(self->allocator, pair->key);
                        pair->key = NULL;
                        if (self->value_deleter != NULL) {
                            self->value_deleter(pair->value);
                        }
                        allocator_free(self->allocator, pair->value);
                        pair->value = NULL;
                        --self->count;
                        return true;
                    }
...
void map_clear(map_t *self) {
    ...
    for (size_t i = 0; i < self->capacity; ++i) {
        if (self->pairs[i].state == PAIR_STATE_FULL) {
            allocator_free(self->allocator, self->pairs[i].key);
            self->pairs[i].key = NULL;
            if (self->value_deleter != NULL) {
                self->value_deleter(self->pairs[i].value);
            }
            allocator_free(self->allocator, self->pairs[i].value);
...
bool map_foreach(map_t *self, bool (*action)(const void *key, void *value, void *data), void *data) {
    ...
    for (size_t i = 0; i < self->capacity && count > 0; ++i) {
        if (self->pairs[i].state == PAIR_STATE_FULL) {
            if (!action(self->pairs[i].key, self->pairs[i].value, data)) {
                return false;
            }
            --count;
        }
